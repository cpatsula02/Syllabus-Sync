    ...
    if any(word in item_lower for word in ['instructor', 'email', 'contact', 'professor']):
        ...
        for email in all_emails:
            ...
            if email_idx >= 0:
                ...
                surroundingtext = document[max(0, email_idx-150):min(len(document), email_idx+len(email)+150)]
...
def find_matching_excerpt(item, document_text):
    ...
    # Enhanced threshold calculation with weighted criteria
    min_score_threshold = 2.0 + (len(item) / 100)  # Increased base threshold for better accuracy
    min_matches = max(2, len(key_terms) // 3)  # Adaptive matches based on number of key terms

    # Calculate semantic relevance score
    semantic_score = sum(len(term) * 0.2 for term in best_matches if len(term) > 4)
    total_score = best_score + semantic_score
    ...
    if total_score >= min_score_threshold or len(best_matches) >= min_matches or has_semantic_match:
