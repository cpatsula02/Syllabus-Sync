...
def process_documents(checklist_path: str, outline_path: str, api_attempts: int = 3, additional_context: str = "") -> Tuple[List[str], Dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            if ENABLE_OPENAI:
                # FIXED: Removed signal-based timeout handling which was causing issues
                # Using the built-in timeout in the API call instead

                # Log what we're doing
                logging.info(f"Using OpenAI API EXCLUSIVELY for analysis (NO fallbacks used)")
                
                try:
                    # Try the OpenAI API - it now has its own timeout handling
